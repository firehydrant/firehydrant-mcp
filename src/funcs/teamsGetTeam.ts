...
> {
  const parsed$ = safeParse(
    request,
    (value$) => GetTeamRequest$zodSchema.parse(value$),
  ...
  const path$ = pathToFunc("/v1/teams/{team_id}")(
    pathParams$,
  );
  ...
  const [result$] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, GetTeamResponse$zodSchema, { key: "TeamEntity" }),
